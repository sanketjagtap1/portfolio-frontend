 ...
 * Usage: <app-icon name="arrow-up-right" [size]="18"></app-icon>
 */
@Component({
  selector: 'app-icon',
  standalone: true,
  ...
  // The whole <svg> is produced as a markup string bound to a host <span>.
  // Binding [innerHTML] directly on an <svg> element breaks server-side
  // prerendering (domino has no SVG innerHTML setter → NotYetImplemented),
  // while innerHTML on an HTML element renders fine on both server and client.
  template: `<span class="app-icon-wrap" aria-hidden="true" [innerHTML]="svg"></span>`,
  styles: [`
    :host { display: inline-flex; line-height: 0; }
    .app-icon-wrap { display: inline-flex; line-height: 0; }
  `],
})
export class IconComponent {
  private _size = 18;
  private _strokeWidth = 1.75;
  private _name = '';
  svg: SafeHtml = '';
  ...
  @Input() set strokeWidth(v: number) { this._strokeWidth = v; this.render(); }
  ...
  private render(): void {
    ...
    const markup =
      `<svg width="${this._size}" height="${this._size}" viewBox="0 0 24 24" fill="none" ` +
      `stroke="currentColor" stroke-width="${this._strokeWidth}" stroke-linecap="round" ` +
      `stroke-linejoin="round">${path}</svg>`;
    this.svg = this.sanitizer.bypassSecurityTrustHtml(markup);
  }
}

// Raw SVG inner markup keyed by name. Brand marks (github/linkedin/x) use fill.
const ICONS: Record<string, string> = {
  ...
  'sparkles': '<path d="M12 3l1.9 5.1L19 10l-5.1 1.9L12 17l-1.9-5.1L5 10l5.1-1.9L12 3Z"/><path d="M19 15l.9 2.6L22 18l-2.1.9L19 21l-.9-2.1L16 18l2.1-.4L19 15Z"/>',
  ...
  // Brand marks (filled)
  'github': '<path fill="currentColor" stroke="none" d="M12 .5C5.37.5 0 5.87 0 12.5c0 5.3 3.44 9.8 8.21 11.39.6.11.82-.26.82-.58v-2.03c-3.34.73-4.04-1.6-4.04-1.6-.55-1.39-1.34-1.76-1.34-1.76-1.09-.75.08-.73.08-.73 1.2.08 1.84 1.24 1.84 1.24 1.07 1.84 2.81 1.31 3.5 1 .11-.78.42-1.31.76-1.61-2.67-.3-5.47-1.34-5.47-5.96 0-1.32.47-2.39 1.24-3.23-.12-.31-.54-1.53.12-3.18 0 0 1.01-.32 3.3 1.23a11.5 11.5 0 0 1 6 0c2.28-1.55 3.29-1.23 3.29-1.23.66 1.65.24 2.87.12 3.18.77.84 1.23 1.91 1.23 3.23 0 4.63-2.8 5.65-5.48 5.95.43.37.81 1.1.81 2.22v3.29c0 .32.22.7.83.58A12 12 0 0 0 24 12.5C24 5.87 18.63.5 12 .5Z"/>',
  ...
  'twitter': '<path fill="currentColor" stroke="none" d="M18.24 2.25h3.31l-7.23 8.26 8.5 11.24h-6.66l-5.22-6.82-5.97 6.82H1.66l7.73-8.84L1.24 2.25h6.83l4.71 6.23 5.46-6.23zm-1.16 17.52h1.83L7.01 4.12H5.05l12.03 15.65z"/>',
};
